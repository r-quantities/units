...
\end{center}
\caption{Plot of 1/consumption against 1/displacement of dataset {\tt mtcars}, illustrating automatic placement of units in axis labels (after conversion to SI)}
\label{fig}
\end{figure}
...
as.units(with(ud_units, yr), with(ud_units, d))
@
where the first answer refers to a common year, and the last to the
interval between 2 successive passages of the sun through vernal
equinox. This illustrates that those who apply unit conversion
should be aware of possible pitfalls. Support for calendars in
udunits seems less developed than in R: the length of a month is
for instance taken as $1/12$ of a year.
...
estimates and appear in axis labels on lattice plots (\cite{lattice})
or ggplots (\cite{ggplot2}).
