...
@
where the first answer refers to a typical year (the mode), and
the last to the interval between 2 successive passages of the
sun through vernal equinox. Udunits was not designed to handle
calendars. This illustrates that those who apply unit conversion
...
#library(units)
#temp_data$Annual_Mean = temp_data$Annual_Mean * with(ud_units, `°C`)
#plot(Annual_Mean~date, temp_data, cex = .8)
