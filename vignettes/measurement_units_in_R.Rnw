...
@
where the unit of change is in degrees Celsius but either {\em per
day} ({\tt date}) or {\em per second} ({\tt time}). For purely
mathematical manipulations, R often strips context from numbers
when it is carried in attributes, the linear algebra routines being
a prime example.
...
from the \code{ud\_units} database, which is part of \CRANpkg{units}, and was
...
as_cf(m^2*s^-1)
@
The \code{plot} and \code{hist} methods add units to default axis labels, an
example is shown in Figure \ref{fig}. For \CRANpkg{ggplot2} plots
\citep{ggplot2}, automatic unit placement in default axis label is provided by package {\bf ggforce} \citep{ggforce}; \code{demo(ggforce)} gives an example.
\begin{figure}
\begin{center}
<<echo=FALSE,fig=TRUE,height=3.8,width=7,eval=FALSE>>=
...
\end{center}
\caption{Plot of 1/consumption against 1/displacement of dataset {\tt mtcars}, illustrating automatic units in default axis labels (after conversion to SI) for base plot (left) and ggplot (right); \code{demo(ggforce)} illustrates how these plots are generated. }
\label{fig}
\end{figure}
...
year \CRANpkg{NISTunits} or \CRANpkg{udunits2} give us an answer
that depends on whether we want a common, leap, Gregorian, Julian,
tropical or siderial year (\cite{lang}, see also \code{demo(year)}).
This illustrates that those who apply unit conversion should be
aware of possible pitfalls. Support for calendars in udunits seems
not as well developed as in R.
...
the ggplot extensions in package {\bf ggforce} that automatically
add units to default ggplot axis labels (Fig 1).
