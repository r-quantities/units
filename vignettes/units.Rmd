...
Units from `udunits2` are defined in a list, `ud_units` and can be used either by attaching the list or using `with`. The latter is preferable since some unit names will conflict with function names, e.g. `min` for minutes and minimum.
