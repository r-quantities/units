...
Mesures) is the "_the intergovernmental organization through which
...
@si describe the SI units, where, briefly, {\em SI units}

* consist of seven base units (length, mass, time \& duration, electric current, thermodynamic temperature, amount of substance, and luminous intensity), each with a name and abbreviation
(Table \ref{tab:si}) 
* consist of {\em derived units} that are formed by products of powers of base units, such as \samp{$m/s^2$}, many of which have special names and symbols (e.g.~angle: 1 rad = 1 m/m; force: 1 N = 1 m kg s$^{-2}$) 
* consist of _coherent derived units_ when derived units include no numerical factors other than one (with the exception of \samp{kg}\footnote{as a base unit, kg can be part of coherent derived units}); an example of a coherent derived unit is 1 watt = 1 joule per 1 second, 
* may contain SI prefixes (k = kilo for $10^3$, m = milli for $10^{-3}$, etc.) 
* contain special quantities where units disappear (e.g., m/m) or have the nature of a count, in which cases the unit is \samp{1}.
...
does this by the \code{conv\_unit} function:
```{r}
...
```
Function \code{conv\_dim} allows for the conversion of units in products or ratios,
e.g.
```{r}
...
[@udunits2] provides an R level interface to the most important
...
ls(2)
```
Dropping the \code{ud} prefix,
\code{is.parseable} verifies whether a unit is parseable
```{r}
ud.is.parseable("m/s")
ud.is.parseable("q")
```
\code{are.convertible} specifies whether two units are convertible
```{r}
ud.are.convertible("m/s", "km/h")
ud.are.convertible("m/s", "s")
```
\code{convert} converts units that are convertible, and throws an error otherwise
```{r}
ud.convert(1:3, "m/s", "km/h")
```
and \code{get.name}, \code{get.symbol} and \code{set.encoding} get name, get symbol
...
The [units](https://cran.r-project.org/package=units) package builds \code{"units"} objects from scratch,
where \code{m}, created by
```{r}
library(units)
m = make_unit("m")
str(m)
...
x1 = 1:5 * m
```
As an alternative to using \code{make\_unit}, we can retrieve units directly
from the \code{ud\_units} database, which is part of [units](https://cran.r-project.org/package=units), and was
...
identical(x1, x3)
```
Although one could attach \code{ud\_units} to use the units directly,
there are over 3000 and this would not only clobber the namespace
but also lead to conflicts, e.g. for \code{T} (Tesla, \code{TRUE})
or \code{in} (inch, reserved R language element). The last form using
\code{with} has the advantage that it can take direct expressions:
```{r}
with(ud_units, m/s^2)
```
...
attr(e, "condition")[[1]]
```
The full set of methods and method groups for \code{units} objects is shown by
```{r}
methods(class = "units")
```
...
z/y
```
However, \code{parse\_unit} parses such units, and \code{as\_cf} returns such unit strings from \code{"units"} objects:
```{r}
parse_unit("m2 s-1")
as_cf(m^2*s^-1)
```
...
[@ggplot2], automatic unit placement in default axis label is provided by package [ggforce](https://cran.r-project.org/package=ggforce) [@ggforce]; \code{demo(ggforce)} gives an example.

```{r fig=TRUE,echo=FALSE,height=3.8,width=7}
library(units)
...
Plot of `1/consumption` against `1/displacement` for dataset
`mtcars`, illustrating automatic units in default axis labels
(after conversion to SI) for base plot (top) and ggplot (bottom);

```r
demo(ggforce)
```
illustrates how these plots are generated.
...
The [units](https://cran.r-project.org/package=units) R package provides a new class, \code{"units"},
...
the help of the udunits library and unit database R package
[units](https://cran.r-project.org/package=units) allows for
...
\code{"data.frame"} slots, but for instance not in `zoo`
...
also \code{demo(year)}).  This illustrates that those who apply
