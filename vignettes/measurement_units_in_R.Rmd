...
set_units(1, m^5/s^4)
```
Another way to print units commonly seen in Climate and Forecast Conventions\footnote{CF, \url{http://cfconventions.org/Data/cf-standard-names/34/build/cf-standard-name-table.html}} is `m2 s-1` for m$^2$/s. These are not R expressions, but as they can be parsed by `parse_unit`:
```{r}
<<<<<<< HEAD
parse_units("m2 s-1", implicit_exponents = TRUE)
=======
as_units("m2 s-1")
>>>>>>> d8459a48c250c043b4ce9d1d804d17c700511fe3
