...
str(m)
```
represents `1 m`, one meter. Other length values are obtained by using this
unit in an expression:
```{r}
x1 = 1:5 * m
```
As an alternative to using `make_unit`, we can retrieve units directly
from the `ud_units` database, which is part of [units](https://cran.r-project.org/package=units), and was
...
identical(x1, x3)
```
Although one could attach `ud_units` to use the units directly,
there are over 3000 and this would not only clobber the namespace
but also lead to conflicts, e.g. for `T` (Tesla, `TRUE`)
or `in` (inch, reserved R language element). The last form using
`with` has the advantage that it can take direct expressions:
```{r}
with(ud_units, m/s^2)
```
A convenience function that directly evaluates a units expression in the
environment `ud_units`, thus avoiding `with`, is `set_units`:
```{r}
set_units(1:5, cm)
library(magrittr)
1:5 %>% set_units(N/s)
...
set_units(1, m^5/s^4)
```
Another way to print units commonly seen in Climate and Forecast Conventions\footnote{CF, \url{http://cfconventions.org/Data/cf-standard-names/34/build/cf-standard-name-table.html}} is `m2 s-1` for m$^2$/s. These are not R expressions, but as they are understood by udunits,
...
z/y
```
However, `parse_unit` parses such units, and `deparse_unit` returns such unit strings from `units` objects:
```{r}
parse_unit("m2 s-1")
